...
class FL1_CPT {
    ...
    public function __construct( $post_type_names, $options = array() ) {
        ...
        if ( is_array( $post_type_names ) ) {
            ...
            foreach ( $names as $name ) {
                ...
                if ( isset( $post_type_names[ $name ] ) ) {
                    ...
                    if($post_type_names['menu_name']) {
                        $this->display_menu_name = $post_type_names['menu_name'];
                    }
        ...
        $this->add_filter( 'enter_title_here', array( $this, 'post_type_placeholder_title' ), 10 , 1);
    }
    ...
    public function register_post_type() {
        ...
        $slug      = $this->slug;
        $menu_name = $this->display_menu_name;

        // Default labels.
        $labels = array(
            'name'               => sprintf( __( '%s', $this->textdomain ), $plural ),
            'singular_name'      => sprintf( __( '%s', $this->textdomain ), $singular ),
            'menu_name'          => sprintf( __( '%s', $this->textdomain ), $menu_name ? $menu_name : $plural ),
        ...
        $defaults = array(
            'labels'                => $labels,
            'public'                => true,
            'description'           => __( 'This is where you can add '.$plural, $this->textdomain),
            'show_ui'               => true,
            'show_in_menu'          => true,
    ...
    */
    ...
    /**
     * Changes post type title placeholder title
     */
    public function post_type_placeholder_title($title) {

        return sprintf(__( '%s title/name',  get_post_type() ), ucfirst(get_post_type()));
